...
/** `clijvm cpu stop <target>` — dump and stop a background recording, then report CPU hotspots. */
class CpuStopCommand(
    private val sessions: SessionStore = SessionStore(),
) : CliktCommand(
    name = "stop",
    help = "Stop a background recording, save it, and print the CPU report.",
) {
    ...
    override fun run() = doStop(target, sessions, ReportView.CPU, format, output)
}
